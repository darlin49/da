...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'SAVE_TRANSLATION') {
    handleSaveTranslation(request.word, request.translation)
      .then(result => sendResponse(result))
      .catch(error => sendResponse({ success: false, error: error.message }));
    return true;
  }
  
  if (request.type === 'GET_TRANSLATIONS') {
    getTranslations()
      .then(translations => sendResponse({ success: true, data: translations }))
      .catch(error => sendResponse({ success: false, error: error.message }));
    return true;
  }
  
  if (request.type === 'DELETE_TRANSLATION') {
